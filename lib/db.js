 ...
 *
 * A tiny document store backed by a JSON file. Every API write goes through
 * here, so the whole prototype has a single source of truth that survives
 * server restarts (Phase 2 of the roadmap: "real data layer").
 */
...
// DATA_DIR lets a host point at a mounted persistent volume. If the filesystem
// is read-only (serverless runtimes), we keep working in memory instead of
// crashing — the demo never dies, it just stops persisting across cold starts.
...
function load() {
  try {
    ...
    // Guard against older/partial files.
    const fresh = seed();
...
function save() {
  try {
    ...
    if (fileWritable) {
      fileWritable = false;
      console.warn(`  !  ${DATA_DIR} is not writable (${err.code || err.message}). Running in memory only — state resets on restart.`);
    }
  }
}
